    ...
    await sequelize.query(`SELECT count(*) From TokenAdministrator where Value=${token}`)
        .then(async function (mes) {
            if (mes[0][0]['count(*)'] === 0) {
                ...
                await sequelize.query(`INSERT INTO Notice(noticeTitle,startTime,endTime,noticeContent,Description) 
                    VALUES(${noticeTitle},${startTime},${endTime},${noticeContent},${Description})`)
                    .then(mes =>{
                        res.json({
                            noticeID:mes[0]
                        })
