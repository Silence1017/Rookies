    ...
    await sequelize.query(`Select UserID From TokenUser where TokenUser.Value=${token} and exists (Select count(*) From TokenUser where Value=${token})`)
        .then(async function (mes) {
            if (mes[0].length === 0) {
               ...
               sequelize.query(`Select OrderID,Destination,Location from ${tableName},User where Order.status = 0 and User.UserID = (Select Distinct PassengerID from ${tableName} where status = 0)`)
               .then(mes=>{
                   res.send(mes[0]);
               })
